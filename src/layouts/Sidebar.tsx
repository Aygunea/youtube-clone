import { ChevronDown, ChevronUp, Clapperboard, Clock, Divide, Film, Flame, Gamepad2, History, Home, Library, Lightbulb, ListVideo, Music2, Newspaper, PlaySquare, Podcast, Radio, Repeat, Shirt, ShoppingBag, Trophy } from "lucide-react"
...
export function Sidebar() {
    ...
    return <>
        ...
        <aside className={`w-56 lg:sticky absolute top-0 overflow-y-auto scrollbar-hidden pb-4 px-2 flex-col gap-2
...
function SmallSidebarItem({ Icon, title, url }: SmallSideBarItemProps) {
    return (
        <a href={url} className={twMerge(buttonStyles({ variant: "ghost" }),
            "py-4 px-1 flex flex-col items-center gap-1 rounded-lg")}>
...
function LargeSidebarSection({
    ...
    return (<div>
        ...
        {showExpandButton && <Button
            ...
            variant="ghost"
            className="p-3 w-full flex  items-center gap-4 rounded-lg"
        >
...
function LargeSidebarItem({ IconOrImgUrl, title, url, isActive = false }: LargeSidebarItemProps) {
    return (
        <a href={url} className={twMerge(buttonStyles({ variant: "ghost" }),
            `p-3 w-full flex  items-center gap-4 rounded-lg
